...
class YOLOLoss(nn.Module):
    ...
    def get_target(self, l, targets, anchors, in_h, in_w):
        ...
        bs          = len(targets)
        # 用于选取哪些先验框不包含物体
        # [bs,3,i,j]--> i,j 可以取(13,13)、(26,26)、(52,52)
        noobj_mask  = torch.ones(bs, len(self.anchors_mask[l]), in_h, in_w, requires_grad=False)    
        # 让网络更加关注小目标
        # [bs,3,i,j]--> i,j 可以取(13,13)、(26,26)、(52,52)
        box_loss_scale  = torch.zeros(bs, len(self.anchors_mask[l]), in_h, in_w, requires_grad=False)   
        # [bs, 3, i, j, 5 + num_class]
        y_true          = torch.zeros(bs, len(self.anchors_mask[l]), in_h, in_w, self.bbox_attrs, requires_grad=False)
        
        for b in range(bs):
            if len(targets[b]) == 0:
                continue
            # targets--> [bs,[[x,y,w,h,class],[x,y,w,h,class],...]]
            batch_target    = torch.zeros_like(targets[b])
            #   计算出正样本在特征层上的中心点 (预处理已经长宽都缩放到了0-1，所以这里乘以in_w、in_h是为了放大到和特征层一样大（13x13）)
            batch_target[:, [0,2]]  = targets[b][:, [0,2]] * in_w
            batch_target[:, [1,3]]  = targets[b][:, [1,3]] * in_h
            batch_target[:, 4]      = targets[b][:, 4]  # 种类
            batch_target            = batch_target.cpu()
            
            # 添加同样的中心坐标->[num_true_box,4]（进行先验框的筛选）
            gt_box      = torch.FloatTensor(torch.cat((torch.zeros((batch_target.size(0), 2)), batch_target[:, 2:4]), dim=1))
            # 添加同样的中心坐标->[9,4]（进行先验框的筛选）
            ...
            #-------------------------------------------------------#
            best_ns = torch.argmax(self.calculate_iou(gt_box, anchor_shapes), dim=-1)   # 这里计算Giou不考虑坐标，只考虑宽高，看该匹配哪些anchor
    ...
    def get_ignore(self, l, x, y, h, w, targets, scaled_anchors, in_h, in_w, noobj_mask):
        ...
        for b in range(bs):           
            ...
            if len(targets[b]) > 0:
                ...
                # anch_ious_max：[3,i,j]--> i,j 可以取(13,13)、(26,26)、(52,52)
